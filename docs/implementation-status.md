...
- 全仓类型检查、单元/接口测试和生产构建通过：Web 16 项、契约 16 项、领域 10 项、Sidecar 24 项、API 67 项，共 133 项测试。
...
- Railway 生产 Smoke 通过签名 HC 事件、经理任务创建、HR 站内进度、经理单次打开复用工作区、企业知识检索、事实确认、画像生成与画像确认链路；通知派发开关保持关闭，等待真实经理飞书身份映射后再开启。
- 生产 Smoke 发现画像已通过权威工具成功落库后，Harness 仍等待最终消息并超时；Sidecar 已改为仅在非对话任务的权威保存工具成功后提前收敛，失败工具结果不收敛，澄清对话保持原行为。
