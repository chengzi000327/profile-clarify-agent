...
- [x] 4.4 Deploy only Railway `api` from the exact final commit; verify generation, confirmation, rollback and concurrency through the complete automated write suite, then verify API health, three-role login, role list/detail, HR visibility, admin Trace and service compatibility through production read-only Smoke; record deployment IDs, service status, Smoke results and rollback commit without creating production artifact versions.
