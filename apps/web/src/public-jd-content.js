...
const toText = (value, fallback = '') => {
  ...
  if (isRecord(value)) {
    for (const key of ['statement', 'text', 'title', 'name', 'description', 'evidence', 'value']) {
...
const toLocation = (value, fallback) => {
  if (Array.isArray(value)) {
...
const normalizeResponsibilities = (value, fallback) => {
  if (!Array.isArray(value)) return fallback;
  const items = value.map((item) => toText(item)).filter(Boolean);
  return items.length ? items : fallback;
};

const normalizeCapabilities = (value, fallback) => {
  if (!Array.isArray(value)) return fallback;
  const items = value.map((item, index) => {
    if (typeof item === 'string') {
      return { id: `capability-${index}`, title: item.trim(), description: '' };
    }
