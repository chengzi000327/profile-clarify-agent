-- The first Railway demo role was created through the intake flow and therefore
-- has a generated UUID rather than seed.ts's stable local UUID. Match that
-- legacy record by its demo tenant and role identity, then bring it onto the HC
-- context and membership contract used by newly created approved roles.
