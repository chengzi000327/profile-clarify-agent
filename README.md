...
- HC 审批事件经过时间窗和签名校验后，幂等创建岗位澄清任务；系统只主动提醒该 HC 的用人经理，HR 在站内查看任务进度，不重复催办。
- Agent 提取的岗位事实先作为草稿显示在原对话消息下，由用人经理确认、修订或驳回；仍有待确认事实时不能生成正式岗位画像，事实变化会使受影响的已确认产物失效并留下审计记录。
- 模拟公司的组织职责、岗位族、职级、历史 JD、画像案例、招聘规范和面试标准会按租户、角色与任务检索后进入模型上下文；候选人简历和面试材料不进入这条岗位澄清检索链路。
...
1. **HC 审批到主动澄清**：Mock HRIS 用签名事件通知审批完成，API 原子创建 HC、经理任务和一条飞书通知。通知只发一次给用人经理；未绑定飞书时记为 `UNBOUND`，不会改发给 HR。经理选择 HC 后任务进入“进行中”，岗位画像确认后任务完成。
2. **岗位事实确认到正式生效**：Agent 只能提出事实草稿，不能替人确认。事实确认卡嵌在原对话中，用人经理负责确认、修订或驳回，HR 和管理员按既有权限查看。待确认事实全部处理后才能生成正式画像；后续改动会让依赖旧事实的已确认产物失效，防止旧结论继续流转。
3. **企业知识到模型上下文**：每次澄清或生成产物前，后端先按租户、测试角色、任务类型、部门和岗位族筛选模拟企业知识，再把最多六条带来源版本的摘要交给 Harness。模型把它们作为参考证据，不能自动变成已确认事实。候选人提取与招聘校准任务明确关闭这条检索，避免把候选人隐私混入岗位澄清上下文。

前端只做三处小改：HC 卡片增加“待澄清 / 已提醒 / 进行中”等小状态；Agent 消息下增加事实确认卡；岗位画像入口显示待处理数量。导航、整体布局、岗位画像正文、HR 共享工作区和 Trace 位置不变。
...
## 主要命令
    ...
    corepack pnpm harness:prepare
    corepack pnpm harness:smoke:runtime
    HC_EVENT_URL=http://localhost:5173/api/v1/integrations/mock-hris/hc-events \
      HC_EVENT_SECRET=<与 API 相同的本地密钥> corepack pnpm hc:demo:event
...
- 企业知识先做租户、角色、有效期和任务类别过滤；模型输出保留来源引用，企业知识不会自动成为正式事实。
- 候选人材料只用于候选人证据提取和招聘校准，永远不进入岗位澄清企业知识检索。
